...
interface UseTaskConflictsReturn {
    ...
    findAvailableSlot: () => { startMinutes: number; endMinutes: number } | null;
}
...
export function useTaskConflicts({
    ...
    const conflictingTasks = useMemo(() => {
        return tasks.filter(task => {
            ...
            return isOverlapping(
                ...
                { startMinutes: task.scheduled.startMinutes, endMinutes: task.scheduled.endMinutes }
            );
        });
    ...
    const findAvailableSlot = (): { startMinutes: number; endMinutes: number } | null => {
        ...
        const hasConflict = busySlots.some(slot => 
            isOverlapping(
                { startMinutes, endMinutes: preferredEnd },
                slot
            )
        );
        ...
        const hasConflictAtStart = busySlots.some(slot => 
            currentTime >= slot.startMinutes && currentTime < slot.endMinutes
        );

        if (!hasConflictAtStart) {
            let endTime = currentTime + duration;
            let hasConflict = busySlots.some(slot => 
                isOverlapping(
                    { startMinutes: currentTime, endMinutes: endTime },
                    slot
                )
            );
        ...
        for (const slot of busySlots) {
            ...
            if (!hasConflict && candidateEnd <= DAY_END) {
                return { startMinutes: candidateStart, endMinutes: candidateEnd };
            }
        }
        ...
        for (const slot of busySlots) {
            if (slot.startMinutes - currentTimeBefore >= duration) {
                return { startMinutes: currentTimeBefore, endMinutes: currentTimeBefore + duration };
            }
            currentTimeBefore = Math.max(currentTimeBefore, slot.endMinutes);
        }

        if (DAY_END - currentTime >= duration) {
            return { startMinutes: currentTime, endMinutes: currentTime + duration };
        }
